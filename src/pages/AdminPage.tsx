...
const QUICK_QUERIES = [
  ...
  { label: 'Attendance', sql: 'select * from attendance order by clock_in_at desc limit 20' },
...
const DEPT_PALETTE = [
  { accent: '#1d4ed8', light: '#dbeafe', border: '#bfdbfe', bg: '#eff6ff' },
  { accent: '#16a34a', light: '#dcfce7', border: '#bbf7d0', bg: '#f0fdf4' },
  { accent: '#7c3aed', light: '#f3e8ff', border: '#e9d5ff', bg: '#faf5ff' },
  { accent: '#c2410c', light: '#ffedd5', border: '#fed7aa', bg: '#fff7ed' },
  { accent: '#0f766e', light: '#ccfbf1', border: '#99f6e4', bg: '#f0fdfa' },
  { accent: '#9333ea', light: '#fae8ff', border: '#f5d0fe', bg: '#fdf4ff' },
]

type Tab = 'users' | 'departments' | 'attendance' | 'timetable' | 'audit' | 'sql'
...
function Avatar({ name, size = 28 }: { name: string | null; size?: number }) {
  return (
    <div style={{ width: size, height: size, borderRadius: '50%', background: 'linear-gradient(135deg,#00b8b8,#0d2d3a)', color: 'white', fontSize: size * 0.32, fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
...
function AdminPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: ps }, { data: ds }, { data: al }] = await Promise.all([
  ...
  async function handleAddUser() {
    if (!newUser.email || !newUser.password || !newUser.full_name) { setAddUserError('Name, email and password are all required.'); return }
  ...
  async function handleDeleteUser(userId: string, name: string) {
    if (!window.confirm(`Delete "${name}"? This permanently removes their account and all data.`)) return
  ...
  async function deleteDepartment(id: string, name: string) {
    ...
    const msg = count > 0 ? `"${name}" has ${count} member(s) who will be unassigned. Delete anyway?` : `Delete department "${name}"?`
  ...
  if (loading) return <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', color: '#6b7280' }}>Loading…</div>
                ...
                {p.id in editingPin ? (
                  <div style={{ display: 'flex', gap: '0.4rem', alignItems: 'center' }}>
                    <input
                      ...
                      autoFocus
                      style={{ width: 72, padding: '0.3rem 0.5rem', border: '1px solid #00b8b8', borderRadius: 6, fontSize: '0.82rem', fontFamily: 'monospace', letterSpacing: '0.2em', textAlign: 'center' }}
                    />
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                    <span style={{ fontFamily: 'monospace', fontSize: '0.85rem', color: p.kiosk_pin ? '#0d2d3a' : '#d1d5db', letterSpacing: '0.12em' }}>
                      {p.kiosk_pin ? '● ● ● ●' : 'No PIN'}
                    </span>
  ...
  return (
    <>
      <style>{`
        .al { min-height: 100vh; background: #f0f2f5; }
        .al-body { display: flex; max-width: 1400px; margin: 0 auto; padding: 1.75rem 1.5rem; gap: 1.5rem; align-items: flex-start; }

        /* Sidebar */
        .al-sidebar { width: 215px; flex-shrink: 0; background: white; border: 1px solid #e5e7eb; border-radius: 14px; padding: 1.25rem 0.75rem; position: sticky; top: 72px; }
        .al-sidebar-label { font-size: 0.68rem; font-weight: 700; color: #9ca3af; letter-spacing: 0.1em; text-transform: uppercase; padding: 0 0.6rem; margin-bottom: 0.5rem; display: block; }
        .al-tab { display: flex; align-items: center; gap: 0.6rem; width: 100%; padding: 0.6rem 0.75rem; border: none; border-radius: 8px; background: transparent; cursor: pointer; font-size: 0.88rem; font-weight: 500; color: #374151; text-align: left; transition: all 0.12s; margin-bottom: 0.1rem; }
        .al-tab:hover { background: #f9fafb; }
        .al-tab.active { background: #0d2d3a; color: white; font-weight: 600; }
        .al-sidebar-stats { margin-top: 1.25rem; padding: 1rem 0.6rem 0; border-top: 1px solid #f3f4f6; display: flex; flex-direction: column; gap: 0.85rem; }
        .al-stat-label { font-size: 0.72rem; color: #9ca3af; display: block; margin-bottom: 0.1rem; }
        .al-stat-value { font-size: 1.4rem; font-weight: 700; color: #0d2d3a; }
        ...
        .al-panel-title { font-size: 1.15rem; font-weight: 700; color: #0d2d3a; margin: 0; }
        ...
        /* Inputs / selects */
        .al-select { padding: 0.32rem 0.6rem; border: 1px solid #e5e7eb; border-radius: 7px; font-size: 0.8rem; background: white; color: #374151; cursor: pointer; }
        .al-select:focus { outline: none; border-color: #00b8b8; }
        .al-input { padding: 0.5rem 0.85rem; border: 1px solid #e5e7eb; border-radius: 8px; font-size: 0.85rem; background: white; color: #374151; width: 100%; box-sizing: border-box; }
        .al-input:focus { outline: none; border-color: #00b8b8; }
        .al-input-sm { padding: 0.32rem 0.6rem; border: 1px solid #e5e7eb; border-radius: 7px; font-size: 0.82rem; background: white; width: 200px; }
        .al-input-sm:focus { outline: none; border-color: #00b8b8; }

        /* Buttons */
        .btn-primary { padding: 0.5rem 1.1rem; background: #0d2d3a; color: white; border: none; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: background 0.15s; white-space: nowrap; }
        .btn-primary:hover { background: #00b8b8; }
        .btn-primary:disabled { opacity: 0.55; cursor: not-allowed; }
        .btn-secondary { padding: 0.5rem 1rem; background: white; color: #374151; border: 1px solid #e5e7eb; border-radius: 8px; font-size: 0.85rem; font-weight: 600; cursor: pointer; transition: all 0.12s; white-space: nowrap; }
        .btn-secondary:hover { background: #f9fafb; }
        .btn-danger { padding: 0.28rem 0.65rem; background: transparent; color: #dc2626; border: 1px solid #fecaca; border-radius: 6px; font-size: 0.73rem; font-weight: 600; cursor: pointer; transition: all 0.12s; }
        ...
        .al-search input { flex: 1; padding: 0.45rem 0.75rem; border: 1px solid #e5e7eb; border-radius: 8px; font-size: 0.85rem; background: white; }
        .al-search input:focus { outline: none; border-color: #00b8b8; }
        ...
        /* Add user form */
        .add-user-panel { border-bottom: 1px solid #e5e7eb; background: #fafeff; padding: 1.25rem 1.4rem; }
        ...
        /* PIN section info */
        .pin-info-bar { display: flex; align-items: center; gap: 0.6rem; padding: 0.65rem 1.1rem; background: #f0fdfa; border-bottom: 1px solid #ccfbf1; font-size: 0.78rem; color: #0f766e; font-weight: 500; }
        ...
        .att-nav-btn:hover { border-color: #00b8b8; color: #00b8b8; }
        .att-date-display { font-size: 0.9rem; font-weight: 700; color: #0d2d3a; min-width: 130px; text-align: center; }
        .att-today-btn { padding: 0.3rem 0.75rem; background: #f0fdfa; border: 1px solid #ccfbf1; border-radius: 7px; color: #0f766e; font-size: 0.78rem; font-weight: 600; cursor: pointer; }
        ...
        .att-summary-value { font-size: 1.5rem; font-weight: 800; color: #0d2d3a; line-height: 1; }
        ...
        .att-dept-badge { font-size: 0.7rem; font-weight: 600; padding: 0.18rem 0.55rem; border-radius: 999px; background: #f0fdfa; color: #0f766e; border: 1px solid #ccfbf1; }
        ...
        .status-out { background: #f0fdfa; color: #0f766e; border: 1px solid #ccfbf1; font-size: 0.68rem; font-weight: 700; padding: 0.18rem 0.5rem; border-radius: 999px; white-space: nowrap; }
        .manual-att-panel { padding: 1.1rem 1.4rem; border-bottom: 1px solid #e5e7eb; background: #fafeff; }
        ...
        .checkbox-row input { width: 16px; height: 16px; cursor: pointer; accent-color: #00b8b8; }
        ...
        .tt-week-label { font-size: 0.9rem; font-weight: 700; color: #0d2d3a; flex: 1; }
        ...
        .tt-day-date.today { color: #00b8b8; }
        ...
        .add-sched-panel { padding: 1.1rem 1.4rem; border-top: 1px solid #e5e7eb; background: #fafeff; }
        ...
        .sql-chip { padding: 0.28rem 0.65rem; background: #f3f4f6; border: 1px solid #e5e7eb; border-radius: 6px; font-size: 0.75rem; font-weight: 600; color: #374151; cursor: pointer; transition: all 0.12s; }
        .sql-chip:hover { background: #0d2d3a; color: white; border-color: #0d2d3a; }
        ...
        .sql-textarea:focus { outline: none; border-color: #00b8b8; }
        ...
        @media (max-width: 960px) {
          ...
          .al-sidebar { width: 100%; position: static; }
      ...
      <div className="al">
        ...
        <div className="al-body">
          ...
          <aside className="al-sidebar">
            <span className="al-sidebar-label">Navigation</span>
            {TABS.map(t => (
              <button key={t.id} className={`al-tab${tab === t.id ? ' active' : ''}`} onClick={() => setTab(t.id)}>
                <span style={{ fontSize: '0.9rem' }}>{t.icon}</span> {t.label}
              </button>
            ))}
            <div className="al-sidebar-stats">
              <div><span className="al-stat-label">Total users</span><span className="al-stat-value">{profiles.length}</span></div>
              <div><span className="al-stat-label">Departments</span><span className="al-stat-value">{departments.length}</span></div>
              <div><span className="al-stat-label">PINs set</span><span className="al-stat-value" style={{ color: '#00b8b8' }}>{profiles.filter(p => p.kiosk_pin).length}</span></div>
              <div><span className="al-stat-label">In today</span><span className="al-stat-value">{attendance.length}</span></div>
              <div><span className="al-stat-label">Shifts this week</span><span className="al-stat-value">{schedules.length}</span></div>
            </div>
          </aside>
          ...
          <main className="al-main">
            ...
            {tab === 'users' && (
              <>
                <div className="al-panel-header">
                  <div><h1 className="al-panel-title">User Management</h1><p className="al-panel-sub">{profiles.length} users · {profiles.filter(p => p.kiosk_pin).length} kiosk PINs assigned</p></div>
                ...
                <div className="al-card">
                  ...
                  {showAddUser && (
                    <div className="add-user-panel">
                      <p style={{ fontWeight: 700, fontSize: '0.88rem', color: '#0d2d3a', margin: '0 0 1rem' }}>New user details</p>
                  ...
                  <div className="pin-info-bar">
                    🔐 Kiosk PINs are used for clock-in on the shared kiosk screen at <strong>/kiosk</strong> — no login required.
                  </div>
              ...
              <>
                ...
                <div className="al-card">
                  <div className="att-date-bar">
                    ...
                    <input type="date" value={attendanceDate} onChange={e => setAttendanceDate(e.target.value)} style={{ padding: '0.35rem 0.6rem', border: '1px solid #e5e7eb', borderRadius: '7px', fontSize: '0.82rem' }} />
                    <button className="att-today-btn" onClick={() => setAttendanceDate(todayStr)}>Today</button>
                  </div>

                  <div className="att-summary-cards">
                    <div className="att-summary-card"><div className="att-summary-value" style={{ color: '#00b8b8' }}>{attendance.length}</div><div className="att-summary-label">Clocked in</div></div>
                  ...
                  {showManualAtt && (
                    <div className="manual-att-panel">
                      <p style={{ fontWeight: 700, fontSize: '0.88rem', color: '#0d2d3a', margin: '0 0 0.85rem' }}>Manual attendance entry</p>
                            ...
                            <tbody>
                              {rows.map(a => (
                                <tr key={a.id}>
                                  ...
                                  <td style={{ fontVariantNumeric: 'tabular-nums', color: a.clock_out_at ? '#374151' : '#d1d5db' }}>{a.clock_out_at ? fmtDatetime(a.clock_out_at) : '—'}</td>
                ...
                <div className="al-card">
                  ...
                  {showAddSched && (
                    <div className="add-sched-panel">
                      <p style={{ fontWeight: 700, fontSize: '0.88rem', color: '#0d2d3a', margin: '0 0 0.85rem' }}>Add scheduled shift</p>
                      ...
                      <div className="add-sched-grid">
                        ...
                        <div><label className="form-label">Start time</label><input type="time" className="al-input" value={newSched.start_time} onChange={e => setNewSched(p => ({ ...p, start_time: e.target.value }))} /></div>
                        <div><label className="form-label">End time</label><input type="time" className="al-input" value={newSched.end_time} onChange={e => setNewSched(p => ({ ...p, end_time: e.target.value }))} /></div>
                  ...
                  <div className="al-table-wrap">
                    <table className="al-table">
                      ...
                      <tbody>
                        {auditLog.map(a => (
                          <tr key={a.id}>
                            <td><span style={{ fontWeight: 600, color: '#0d2d3a' }}>{ACTION_LABELS[a.action] ?? a.action}</span></td>
            ...
            {tab === 'sql' && (
              <>
                <div className="al-panel-header"><div><h1 className="al-panel-title">SQL Console</h1><p className="al-panel-sub">Dev-only — admin access required</p></div></div>
                <div className="al-card">
                  <div className="sql-panel">
                    ...
                    <div className="sql-actions">
                      <button className="btn-primary" style={{ background: '#00b8b8' }} onClick={runSql} disabled={sqlRunning}>{sqlRunning ? '⏳ Running…' : '▶ Run query'}</button>
